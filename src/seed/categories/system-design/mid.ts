...
>[] = [
    // Microservices
    {
        ...
        answers: [
            ...
            "When you have dozens of services that can scale up and down dynamically, you can't hardcode where they live. Service discovery solves this by providing a registry where services register themselves on startup and deregister on shutdown. When service A needs to call service B, it asks the registry for B's current addresses rather than having them configured statically. There are two flavors: client-side discovery, where your service queries the registry directly and picks an instance; and server-side discovery, where you call a load balancer that does the lookup for you. Tools like Consul, etcd, and Eureka provide this. If you're on Kubernetes, it has service discovery built in through its DNS and Service abstractions - you just call a service by name and Kubernetes routes to an available pod."
        ],
    },
    {
        ...
        answers: [
            ...
            "Benefits include independent deployment - teams ship their services without coordinating massive releases. You get targeted scaling - scale the overloaded service, not everything. Fault isolation means one service crashing doesn't take down the whole system. Teams can pick the best tool for their domain. The challenges, though, are real. You've traded code complexity for operational complexity. Debugging a request that spans six services is harder than stepping through a monolith. You need investment in observability: distributed tracing, centralized logging, metrics. Data consistency becomes your problem since you can't do cross-service transactions easily. You need CI/CD maturity to manage many deployables. Integration testing is harder. Honestly, microservices solve organizational scaling problems more than technical ones - they make sense when Conway's Law is pushing you that way."
        ],
    },
    {
        ...
        answers: [
            ...
            "In distributed systems, you often can't have immediate consistency across all nodes without sacrificing availability or performance. Eventual consistency is the tradeoff: data will be consistent eventually, but there's a window where different parts of the system might see different values. For example, after you post a tweet, some users might not see it for a few seconds while it propagates through caches and replicas. The CAP theorem explains why: you can't have perfect consistency, availability, and partition tolerance simultaneously. Most internet-scale systems choose availability and accept eventual consistency. The key is designing your application to handle this gracefully - use idempotent operations, design UIs that work with slightly stale data, and be careful about operations where temporary inconsistency could cause real problems like double-charging a customer."
        ],
    },
    {
        ...
        answers: [
            ...
            "Since services evolve independently, you need to change APIs without breaking existing consumers. The main strategies are URL versioning - putting the version in the path like /api/v2/users - or header versioning where clients pass a version header. URL versioning is more explicit and easier to debug since you can see the version in logs and browser URLs. The best approach is to avoid breaking changes when possible: adding fields is usually safe, removing or renaming fields is not. When you must break compatibility, run old and new versions in parallel and give clients a migration window. Have a clear deprecation policy and communicate it. The tricky part is knowing how many versions to support - too many versions becomes a maintenance nightmare, but sunsetting too fast frustrates clients. Usually supporting the current and previous major version is a reasonable balance."
        ],
    },
    {
        ...
        answers: [
            ...
            "Imagine service B goes down and service A keeps calling it, waiting for timeouts. Those requests pile up, A's threads get exhausted, and now A is down too. That's a cascading failure. A circuit breaker prevents this by tracking failure rates. When failures exceed a threshold, it 'trips' and immediately fails subsequent requests without even trying - fail fast instead of fail slow. After a cooldown period, it enters a half-open state, letting a few test requests through. If they succeed, the circuit closes and normal traffic resumes. If not, it opens again. This gives the downstream service breathing room to recover instead of being hammered with doomed requests. Libraries like Resilience4j make this easy to implement. Combined with timeouts, retries with backoff, and fallbacks, circuit breakers are essential for building resilient distributed systems."
        ],
    },
    {
        ...
        answers: [
            ...
            "You need three pillars of observability: metrics, logs, and traces. Metrics are numerical measurements over time - request rate, error rate, latency percentiles, CPU usage. Prometheus or Datadog collect these, and you build dashboards and alerts on them. Logs are events with context - you need centralized logging with something like the ELK stack because logs are scattered across many containers. Include correlation IDs so you can trace a request across services. Traces show the journey of a single request through your system - tools like Jaeger or Zipkin visualize this as a waterfall, showing where time was spent. You also need health endpoints for each service so orchestrators like Kubernetes know when to restart things. The goal is answering 'what's broken and why' quickly. When an alert fires, you look at dashboards, then logs, then traces to pinpoint the issue."
        ],
    },
    ...
    // Concurrency
    {
        ...
        answers: [
            ...
            "Deadlock is when two or more threads are stuck forever, each holding a resource the other needs. Thread A holds lock X and waits for lock Y; thread B holds lock Y and waits for lock X. Neither can proceed. Four conditions must all be true for deadlock: mutual exclusion on resources, holding one resource while waiting for another, no preemption, and circular wait. Break any one to prevent deadlock. The practical approach is consistent lock ordering - always acquire locks in the same order everywhere in your code. If everyone acquires lock X before lock Y, circular wait can't happen. Other strategies: use timeouts instead of waiting forever, use try-lock with backoff, or avoid locks entirely with lock-free algorithms. Databases handle this by detecting deadlocks and killing one transaction. In application code, prevention through ordering is more reliable than detection."
        ],
    },
    {
        ...
        answers: [
            ...
            "JavaScript is single-threaded but highly concurrent thanks to its event loop. The core mechanism is non-blocking I/O with callbacks - when you make a network request, you provide a callback that runs when the response arrives, meanwhile the thread handles other work. Promises abstracted callbacks into chainable objects, and async/await made async code read like sync code while still being non-blocking. For CPU-bound work that would block the main thread, Web Workers provide true parallelism - separate threads that communicate via message passing, isolated from the main thread's memory. Node.js added worker_threads for the same purpose server-side. There's also SharedArrayBuffer and Atomics for shared memory between workers, but that's specialized. The single-threaded event loop model avoids most traditional concurrency bugs like race conditions on shared memory, though you still get logical races with async operations if you're not careful about state management."
        ],
    },
    {
        ...
        answers: [
            ...
            "They're two strategies for handling concurrent data access. Pessimistic locking grabs an exclusive lock when you read data - no one else can touch it until you're done. It guarantees no conflicts but limits concurrency; everyone waits. Optimistic locking doesn't lock at read time. Instead, the record has a version number. You read version 5, make changes, and when you save, the database checks if it's still version 5. If someone else changed it, your update fails and you retry. This allows much better concurrency since reads don't block each other, but you must handle update failures gracefully. Use pessimistic locking when conflicts are common or retrying is expensive - like booking the last seat on a flight. Use optimistic locking for typical web applications where conflicts are rare and users can just retry. Most ORMs support both through SELECT FOR UPDATE and version columns."
        ],
    },
    // Architecture/Design Patterns
    {
        ...
        answers: [
            ...
            "A repository acts as an in-memory collection of domain objects, hiding the database behind a clean interface. Instead of SQL queries throughout your code, you have a UserRepository with methods like findById(), findByEmail(), and save(). Your business logic treats it like a collection and doesn't know if it's backed by Postgres, MongoDB, or an in-memory store. This separation has big benefits: your domain code stays clean and focused on business rules, testing is easy because you can swap in a fake repository, and switching databases becomes a matter of writing a new repository implementation. It works beautifully with ORMs and is a cornerstone of Domain-Driven Design. The risk is repository bloat - every unique query becomes a new method. Keep repositories focused on common access patterns and consider query objects or specifications for complex dynamic queries."
        ],
    },
    {
        ...
        answers: [
            ...
            "An adapter wraps an existing class to make it work with an interface your code expects - like a travel power adapter that lets a US plug work in a European outlet. Say you're using Stripe for payments and want to add PayPal. Rather than littering your code with payment-provider-specific calls, you define a PaymentProcessor interface and create StripeAdapter and PayPalAdapter that implement it. Each adapter translates your interface's method calls to the vendor's specific API. Your business logic works against the common interface and doesn't care which provider is behind it. This pattern is essential when integrating third-party libraries, working with legacy code, or building systems where you want to swap implementations without changing calling code. The adapter handles the translation, whether that's method renaming, parameter reordering, or data format conversion."
        ],
    },
    {
        ...
        answers: [
            ...
            "Decorators let you wrap objects to add behavior without changing the original class. The decorator implements the same interface as the object it wraps, so it can be used transparently. You can stack decorators - wrap a Logger with an EncryptionDecorator, then with a CompressionDecorator. Each adds its behavior and delegates to the wrapped object. The classic example is Java's I/O streams: you wrap a FileInputStream with BufferedInputStream with GZIPInputStream, each adding functionality. In React, higher-order components are decorators - they wrap a component and add props or behavior. The benefit over inheritance is flexibility: with inheritance, you'd need a class for every combination of behaviors. With decorators, you compose them dynamically. Python's @decorator syntax is related but works differently - it's function wrapping rather than the object-oriented pattern, though the spirit is similar."
        ],
    },
    {
        ...
        answers: [
            ...
            "Strategy replaces conditionals with polymorphism. Instead of a giant switch statement choosing between algorithms, you define an interface and create a class for each algorithm. The context holds a reference to the strategy interface and delegates to it. At runtime, you inject whichever strategy you need. For example, a pricing engine might have different discount strategies - percentage off, buy-one-get-one, tiered pricing. Each is a separate class implementing DiscountStrategy. The engine doesn't know which one it's using; it just calls calculateDiscount(). This makes adding new strategies trivial - create a new class, no existing code changes. It also makes testing clean since each strategy is isolated. The pattern is everywhere: payment processors, compression algorithms, validation rules, routing strategies. In languages with first-class functions, you can often use functions directly instead of classes, but the concept is the same."
        ],
    },
];
